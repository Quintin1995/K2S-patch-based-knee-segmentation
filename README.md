...
- **Mirror Padding**: 
  - **Purpose**: Mirror padding is applied to handle edge effects in the segmentation. Without padding, predictions at the image boundaries can be less accurate due to the limited contextual information available to the model at these locations.
  - **Implementation**: The script mirrors the image data at the borders, providing additional context to the model during prediction. This results in more accurate and smoother predictions at the edges.
  - **Formula**: If `I` is the input image and `P` is the padded image, mirror padding can be expressed as:
    \[
    P(x, y, z) = I(\text{mirror}(x), \text{mirror}(y), \text{mirror}(z))
    \]
    where `mirror()` reflects the coordinate values at the boundaries.
...
- **Self-Ensembling**: 
  ...
  - **Formula**: The final prediction for a voxel can be represented as:
    ...
    \]
    where `N` is the number of overlapping predictions and `y_i` is the prediction from the i-th window.

- **Weighted Dice Loss Application**:
  - **Purpose**: This step adjusts the segmentation based on the weighted Dice coefficient, which is particularly useful for ensuring the accuracy of smaller structures like cartilage.
  - **Implementation**: During the initial segmentation phase, a weighted Dice loss function was used to train the model. The post-processing script revisits these weights to ensure that smaller but important structures are given the necessary attention in the final segmentation.
  - **Formula**: The Dice coefficient is calculated as:
    \[
    \text{Dice} = \frac{2 |A \cap B|}{|A| + |B|}
    \]
    where `A` is the predicted segmentation and `B` is the ground truth. The weighted version adjusts the contribution of different classes based on their size and clinical importance.
